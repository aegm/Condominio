...
class factura
{
	...
	{
            if (isset($data))
                $completar_sql = " and b.id_condominio = $data";
            
            
            $consulta=$this->db->query("SELECT a.*, b.fac_descripcion 
                                        FROM facturas a, rubro_gastos b 
                                        WHERE a.id_rubro = b.id_rubro $completar_sql 
                                        ORDER BY ID_FACTURA DESC");
            
            if($consulta->num_rows==0){
			$this->mensaje = "No se encontraron FACTURAS...";
			$this->msgTipo = "aviso";
			$this->estatus = false;
        ...
        public function agregar($id_rubro, $fac_num, $fac_fecha, $fac_descripcion, $fac_monto, $fac_periodo)
        {
            $query = "insert into facturas (id_rubro, fac_num, fac_fecha, fac_descripcion, fac_monto, fac_periodo, fac_user_crea)
                                     VALUES($id_rubro, '$fac_num', '$fac_fecha', '$fac_descripcion', '$fac_monto', '$fac_periodo', ".$_SESSION[SISTEMA]['id_persona'].")";
        ...
        public function listarFacturaTorre($data)
	{
            if (isset($data))
                $completar_sql = "where id_condominio = $data";
            
            
            $consulta=$this->db->query("SELECT * FROM v_facturas_x_torre $completar_sql ORDER BY ID_FACTURA");
